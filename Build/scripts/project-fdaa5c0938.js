...
/**
 * 改变 运行方式 任务状态
 * run_type：cron | deploy
 */
function update_run_type_status(pro_name, nginx_api_proxy, run_type) {
    if(run_type == "cron"){
        var run_type_name = "定 时 任 务";
    }else{
        var run_type_name = "部 署 测 试";
    }
    swal({
        title: "改 变 " + run_type_name + " 状 态 ?",
        text: "",
        type: "warning",
        showCancelButton: true,
        confirmButtonText: "确定",
        ...
        if (isConfirm) {
            // 调用ajax请求(同步)
            var request_url = "/" + nginx_api_proxy + "/API/set_run_type_status/" + pro_name + "/" + run_type
            var response_info = request_interface_url_v2(url=request_url, method="GET", async=false);
            if(response_info != "请求失败"){
                var msg = response_info.msg;
                if (msg.search("成功") != -1){
                    if(response_info.data.new_run_type_status == true){
                        $("#" + run_type + "_status_btn").html(run_type_name + " 已 开 启");
                        $("#" + run_type + "_status_btn").attr('class', "btn btn-success");
                    }else{
                        swal({text: msg, type: "success", confirmButtonText: "知道了"});
                        $("#" + run_type + "_status_btn").html(run_type_name + " 已 关 闭");
                        $("#" + run_type + "_status_btn").attr('class', "btn btn-danger");
                    }
                    swal({text: msg, type: "success", confirmButtonText: "知道了"});
                    setTimeout(function(){location.reload();}, 1000);
                }
